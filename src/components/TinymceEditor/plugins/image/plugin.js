...
(function () {
    ...
    var makeTab = function (_info, defaultItems=[]) {
      var items = [{
          type: 'dropzone',
          name: 'fileinput'
        },
    ...
    var makeDialogBody = function (info) {
      if (info.hasAdvTab || info.hasUploadUrl || info.hasUploadHandler) {
        const { items } = MainTab.makeTab(info)
        var tabPanel = {
          type: 'tabpanel',
          tabs: flatten([
            // [MainTab.makeTab(info)],
            info.hasAdvTab ? [AdvTab.makeTab(info)] : [],
            info.hasUploadTab && (info.hasUploadUrl || info.hasUploadHandler) ? [UploadTab.makeTab(info, items)] : []
          ])
        };
        return tabPanel;
      } else {
